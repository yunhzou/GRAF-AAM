...
\caption{\textbf{Fragment search branches; symmetry stays compressed.}
(a) An ethanol fragment grows while its compatible placements are refined. At saturation, different target atom sets in ethyl lactate define separate continuations; two admissible placements are shown. Pale continuation edges indicate subsequent search schematically.
(b) At a fragment contact in methyl acetate hydrolysis, the original branch retains O4 in fragment A. A new branch gives B priority, rematches the contacting region, and regrows displaced assignments.
(c) Within one hydrolysis fragment pair, exchanging the oxygen images preserves matching at $\tau_{\mathrm{iso}}=1$ but changes two heavy-atom events to four at threshold $0.5$. Dashed arrows denote event projection, not additional search branches. Saved constraints govern allowed symmetry actions.
The molecular examples use formal bond orders and omit hydrogen transfers; benchmark search and scoring include explicit H.}
\label{fig:algorithm}
\end{figure}
...
\begin{table}[tb]
\centering\small
\caption{\textbf{Default released implementations on Golden.} Counts and percentages use all 1,851 reactions, including failures. ``First'' evaluates the first explicit output; ``any'' evaluates every explicitly returned candidate. SLAP output order is not asserted to be a top-one ranking, and its compressed label families are not exhaustively decoded here. Invalid counts refer to returned candidates; failed counts refer to reactions with no mapped output after the recorded execution retries.}
