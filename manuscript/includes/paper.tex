...
\begin{table}[tb]
\centering\small
\caption{\textbf{GRAFT recovery by endpoint-size orientation.} Every entry uses all 1,851 Golden records and the single-edge sweep. Size is the explicit-atom count. On equal sizes, the first policy uses the stored input order and the second uses its reverse. All directional verdicts at these seed settings are resolved; bidirectional recovery checks the union of both outputs.}
