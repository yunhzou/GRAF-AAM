...
\caption{\textbf{Reference recovery and search cost.} (a) Golden family recovery with sweeps for each GRAFT seed setting and SLAP. Unswept controls are reported in \Cref{tab:seeds}. Every percentage uses all 1,851 records. (b) Mean CPU for smaller-first, larger-first, and bidirectional search on the same \CommonCases{} completed reactions on the Mac. Reference verification is excluded; the procedures have different search objectives, output cardinalities, and I/O timing scopes.}
\label{fig:golden}
\end{figure}
...
\begin{table}[tb]
\centering\small
\caption{\textbf{Recovery and runtime by endpoint-size orientation.} All rows use the sweep. Recovery uses all 1,851 Golden records; mean, median, and 95th-percentile CPU seconds per reaction use the same \CommonCases{} completed Mac reactions. SLAP unions binary and weighted modes within each direction. Size counts explicit atoms; equal-size ties use stored order for smaller-first and its reverse for larger-first. Bidirectional cost sums both searches, not parallel wall latency. GRAFT includes checkpoint I/O and symmetry finalization; SLAP includes graph construction, mapping, and export but excludes I/O. Reference verification is excluded.}
