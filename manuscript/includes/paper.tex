...
Local competition expands the retained event-class set by \CompetitionClasses{} classes across \CompetitionCases{} reactions within the evaluated windows. One added event class in one reaction requires decoding internal family shuffles and is absent from the repair representatives. This demonstrates why final candidate extraction must retain correlated alternatives rather than score only one witness. Each reaction's event window ends one event above the larger of its native-SLAP and SLAP-sweep minima. GRAFT returns \DecodedClasses{} reaction-specific classes across these windows. All accepted competition families were completely decoded within their corresponding event windows; this does not imply that every search trajectory was explored.
...
\caption{\textbf{GRAFT outputs and decoding cost on 140 coordinate reactions.} The default one-seed sweep is used with fragment competition.
(a) GRAFT returns \DecodedClasses{} distinct reaction-specific event classes within the fixed windows; competition adds \CompetitionClasses{} in \CompetitionCases{} reactions.
(b) GRAFT recovery of minimum-event classes among returned SLAP witnesses. All compared classes are covered in 139/140 reactions; the remaining reaction lacks two sweep alternatives at the same four-event minimum.
(c) GRAFT ordered and unordered fragment-pair branch counts; the diagonal denotes equality. The highlighted case falls from 52,669 to 2,856. These are not counts of explicit mappings or event classes.
(d) Complete GRAFT catalogue construction and window decoding on three workers: wall time, aggregate CPU time, and peak combined memory. Search and competition are excluded; wall time was measured under shared load.}
