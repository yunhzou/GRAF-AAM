...
async function main(){
 ...
 await page.evaluate(()=>{const d=window.film.data;if(d.decoded_candidates.length!==2||new Set(d.decoded_candidates.map(x=>x.event_class)).size!==2)throw Error('Final event cards must be canonical-unique');if(d.paths[1].event_class!==d.paths[2].event_class)throw Error('Baseline witness C must merge with B');});
 ...
 fs.writeFileSync(path.join(dir,'browser-validation.json'),JSON.stringify({status:errors.length?'failed':'passed',errors,preview_times:[0,3,6,7.2,7.6,8.2,8.6,9.5,11.2,13.5,19],branch_focus_check:true,sampled_timeline_injections:241,viewport:[1440,900],mode},null,2));
