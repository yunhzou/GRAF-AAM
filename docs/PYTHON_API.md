...
The development branch adds `graft.chirality.select_chiral_witness(decoded, candidate)` for coordinate/index-orientation selection directly from the current signed-event decoder. It preserves concrete bond events and saved family constraints; see [the chirality contract and example](CHIRALITY.md). It returns a feasible witness, not a global RMSD optimum.
