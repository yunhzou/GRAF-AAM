...
The development branch adds `graft.chirality.select_chiral_witness(decoded, candidate)` for coordinate/index-orientation selection directly from the current signed-event decoder. It preserves concrete bond events and saved family constraints; see [the chirality contract and example](CHIRALITY.md). The default requires all defined coordinate orientations across the saved family union. `query_chiral_witness(decoded, candidate, assignments)` checks an exact mapping or partial assignments in that same filtered subset. Infeasibility is reported as `forbidden`, timeouts as `unknown`, and explicitly relaxed outcomes as `relaxed`. No global geometry ranking is performed.

**TODO: a general stereochemistry-preserving AAM search/decoder contract.** Element/WBO inputs do not encode CIP labels, E/Z geometry, formal charge, or isotope labels as first-class constraints. Do not claim raw AAM preserves them. The legacy `select_rp_mappings` stage uses a different, potentially relaxed index-orientation policy; its zero-violation count is not a strict preservation certificate. Use the current `graft.chirality` selector and query for the explicit contract. Its subsequent fixed-correspondence rigid fit is diagnostic only. That downstream check does not supply a general stereochemical guarantee for the event notebook. Fixed-query matching can accept richer node policies, but the full AAM endpoint contract still needs that extension.
