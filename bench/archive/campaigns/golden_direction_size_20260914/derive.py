...
out=dict(denominator=1851,definition='Endpoint size is total explicit-atom count, including hydrogen. Smaller-first uses the original input orientation on equal-size endpoints; larger-first uses its reverse. Bidirectional is their union. No correspondence labels used to select orientation.',source_hashes={str(src.relative_to(a.repo)):sha(src),str(manifest.relative_to(a.repo)):sha(manifest),'src/graft/search_orientation.py':sha(a.repo/'src/graft/search_orientation.py')},methods=methods,no_search_reruns=True)
