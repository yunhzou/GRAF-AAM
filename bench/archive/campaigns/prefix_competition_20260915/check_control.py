...
name='rxn_core._control_competition';spec=importlib.util.spec_from_file_location(name,S/'competition-original.py');module=importlib.util.module_from_spec(spec);sys.modules[name]=module;spec.loader.exec_module(module)
a=read_aam_checkpoint(PACKAGE.parent/'unrestricted_competition_20260915/inputs/case64.pkl.gz')
...
for x,y in zip(old.offers,new.offers):
 y=dict(y);y.pop('proposal_mode');y.pop('prefix');assert x==y
