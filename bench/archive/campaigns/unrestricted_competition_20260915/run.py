...
def worker(case,mode,stage):
 ...
 if stage=='search':
  c=compete_fragments(a,CompetitionConfig(proposal_mode=mode))
...
if __name__=='__main__':
 if len(sys.argv)>1 and sys.argv[1]=='worker':worker(int(sys.argv[2]),sys.argv[3],sys.argv[4])
  ...
  cases=[6,11,59,64,101];modes=['local','anchored','free','extended'];rows=[]
  ...
  with ThreadPoolExecutor(max_workers=2) as pool:
   futures=[pool.submit(launch,c,m,'decode') for c in cases for m in ['none',*modes] if m=='none' or json.loads((S/'runs'/str(c)/m/'search-execution.json').read_text())['status']=='passed']
   for f in as_completed(futures):rows.append(f.result())
