...
def publish(args):
    report(args)
    out=Path('reports/elementary140_output_comparison_20260908');out.mkdir(exist_ok=True)
    for name in ('summary.json','submission.json','refine_submission.json','compare_driver.py','refine_driver.py'):
        shutil.copy2(args.run/name,out/name)
    shutil.copy2(__file__,args.run/'report_driver.py')
    with tarfile.open(out/'per_case_mappings.tar.gz','w:gz') as archive:
        for i in range(140):
            for suffix in ('.json','.refined.json'):
                archive.add(args.run/f'{i}{suffix}',arcname=f'{i}{suffix}')
    with (out/'per_case.csv').open('w',newline='') as stream:
        writer=csv.writer(stream)
        writer.writerow(['index','name','aam_saved_min_events','slap_saved_family_min_events',
                         'shared_best_heavy_pattern','aam_broken','aam_formed','aam_order_changed',
                         'slap_broken','slap_formed','slap_order_changed'])
        for i in range(140):
            row=json.loads((args.run/f'{i}.refined.json').read_text())
            a=row['aam_best_example']['events'];s=min(row['slap'],key=lambda x:x['events']['total'])['events']
            writer.writerow([i,row['name'],a['total'],s['total'],row['best_aam_heavy_pattern_matches_any_slap_modulo_score_preserving_symmetry'],
                *(a[k] for k in ('broken','formed','order_changed')),*(s[k] for k in ('broken','formed','order_changed'))])
    jobs=[json.loads((args.run/f).read_text())['job'] for f in ('submission.json','refine_submission.json')]
...
if __name__=='__main__':
    ...
    parser.add_argument('command',choices=('compare','report','submit','refine','refine_all','publish'))
