...
ROOT=Path('/Users/yunhengz/Documents/Codex/2026-09-11/aa');CODE=ROOT/'work/current-validation/engine';OUT=ROOT/'outputs/fragment_competition_experiment'
...
def child(case):
 ...
 result=dict(case=case,operation_budget=BUDGET,config=asdict(cfg),selected_parents=selected,counts=dict(counts),patterns=patterns,offers=offers,pending=len(queue),search_wall_seconds=search_wall,search_cpu_seconds=search_cpu,total_wall_seconds=time.perf_counter()-total_start,total_cpu_seconds=time.process_time()-total_cpu,archive=str(archive),archive_sha256=hashlib.sha256(archive.read_bytes()).hexdigest(),scope='Fresh fixed-budget search, no reference results loaded. Accepted completion archives await complete final-family decoding.')
